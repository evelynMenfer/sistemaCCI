...
class Master extends DBConnection
{
	...
	function save_company()
	{
		...
		if ($save) {
			$resp['status'] = 'success';
			if (empty($id)) {
				$res['msg'] = "Empresa guardada con éxito.";
				$id = $this->conn->insert_id;
			} else {
				$res['msg'] = "Empresa actualizada con éxito.";
			}
			$this->settings->set_flashdata('success', $res['msg']);
		} else {
			$resp['status'] = 'failed';
	...
	function save_po()
	{
		...
		if ($resp['status'] == 'success') {
			if (empty($id)) {
				$this->settings->set_flashdata('success', " Nueva Cotización guardada correctamente.");
			} else {
				$this->settings->set_flashdata('success', " Cotización de producto actualizada correctamente.");
			}
		}
	...
	function delete_po()
	{
		...
		if ($del) {
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', "Detalles de Cotización eliminados con éxito.");
			if ($bo->num_rows > 0) {
